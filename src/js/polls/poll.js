...
function loadPoll() {
    ...
    if (currentPoll.info_text != "" || currentPoll.info_text_2 != "") {
        // Gets the info text paragraph element
        let infoText    = getElement("poll-info-text"),
            infoText2   = getElement("poll-info-text-2");
        
        // Sets info text
        infoText.innerHTML  = currentPoll.info_text;
        infoText2.innerHTML = currentPoll.info_text_2;
    } else {
...
function loadUserPolls(callback = null) {
    ...
    if (submittedPolls == null && nonSubmittedPolls == null) {
        ...
        alert("Unable to retrieve forms, please try again.");
...
function setNewPollData(poll, selectValues = false) {
    ...
    let pollData = [
        ...
        [poll.info_text_2 != null ? poll.info_text_2 : ""],
...
function createPollBlockElement(data, index = 0, onclick = null) {
    ...
    // Adds title
    pollTitle.innerHTML = `Form #${index}`;
...
function addPollOptionsListener(placeholder = "Alternative") {
    // Parameter values
